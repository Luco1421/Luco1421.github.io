window.addEventListener('message', async e => {
    ...
    if (force_mp4) console.log('[CR Premium] Forzando MP4 (chromecast workaround)');

    // Obter streams
    const streamlist = video_config_media['streams'];

    if (!streamlist || streamlist.length === 0) {
        displayError(`No fue posible obtener los streams del video.`);
        return;
    }
    ...
    function updateWebVideoCasterAnchor() {
        ...
        console.log(`[CR Premium] Definio el WVC en calidad ${r[quality]}p`);
    ...
    function getSourceLocale() {
        try {
            ...
            displayError(`Os cookies ${linkIssue('estan desactivados', '51#issuecomment-1190684190')}!</code>`);
            throw err;
        }
    }
